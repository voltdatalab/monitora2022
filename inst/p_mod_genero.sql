CREATE OR REPLACE FUNCTION azmina_monitora.p_mod_genero(datum timestamp without time zone DEFAULT (CURRENT_DATE - '7 days'::interval), datum2 timestamp without time zone DEFAULT CURRENT_DATE, retw boolean DEFAULT NULL::boolean, OUT data_tweet date, OUT genero text, OUT qtd_tweets numeric, OUT total_dia numeric, OUT perc_total numeric, OUT qtd_tweets_off numeric, OUT total_dia_off numeric, OUT perc_total_off numeric)
 RETURNS SETOF record
 LANGUAGE plpgsql
AS $function$
begin
	RETURN query
with total_data_genero as (
SELECT    cast(t.created_at - interval '3 hours' AS date) AS data_tweet,
          p.genero_f_m                                    AS genero,
...
GROUP BY  cast(t.created_at - interval '3 hours' AS date),
          p.genero_f_m
),
total_dia as (
...
select total_data_genero.data_tweet,total_data_genero.genero,total_data_genero.qtd_tweets,total_dia.qtd_tweets as total_dia
,
	case total_dia.qtd_tweets
		when 0 then 0
		else cast( total_data_genero.qtd_tweets /
          total_dia.qtd_tweets AS numeric) * 100.00
    end AS perc_total,

          total_data_genero.qtd_tweets_off,total_dia.qtd_tweets_off as total_dia_off
,
	case total_dia.qtd_tweets_off
		when 0 then 0
		else cast( total_data_genero.qtd_tweets_off /
          total_dia.qtd_tweets_off AS numeric) * 100.00
	end AS perc_total_off

from total_data_genero
join total_dia on total_data_genero.data_tweet = total_dia.data_tweet
